...
def load_model():
    model = tf.keras.models.load_model("Flower-Recongnition")
    return model
...
def url_uploader():
    ...
    if path is not None:
        ...
        st.image(image, caption="Classifying the Flower", use_column_width=True)
...
def file_Uploader():
    ...
    if not file:
        show_file.info("Upload a picture of the Flower you want to predict.")
        return
